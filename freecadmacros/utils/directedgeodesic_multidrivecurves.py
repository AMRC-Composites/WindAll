...
def drivegeodesicRI(gbStart, drivebars, tridrivebarsmap, LRdirection=1, sideslipturningfactor=0.0, MAX_SEGMENTS=2000, maxlength=-1):
    print(f'Analysing drivegeodesicRI')
    ...
    while True:
        ...
        #CHANGE TO DIFFERENT DRIVECURVE (start and end at different ones?.....)
...
def drivegeodesicRI_multidrivecurve(gbStart, drivebarsstart, tridrivebarsmapstart, drivebarsend, tridrivebarsmapend,
                                    LRdirection=1, sideslipturningfactor=0.0, MAX_SEGMENTS=2000, maxlength=-1):
    #initialises first two points of gbs located on drivecurve and distance dlength from drivecurve at angle 'dsangle'
    gbs = [ gbStart, gbStart.gbForebarC ]
    #calculates dlength for distance between first two points
    dlength = (gbs[1].pt - gbs[0].pt).Len()
    ...
    while True:
        ...
        #check if reached end (back at drivecurve). drivecurveintersectionfinder in geodesicutils
        #CHANGE TO DIFFERENT DRIVECURVE (start and end at different ones?.....)
        gbEnd = drivecurveintersectionfinder(drivebarsend, tridrivebarsmapend, gbs[-1], gbFore, LRdirection=LRdirection)
        if gbEnd:
            gbs.append(gbEnd)
            break
        gbs.append(gbFore)
...
def directedgeodesicmultidrivecurve(combofoldbackmode, sketchplane_or_drivecurve1,sketchplane_or_drivecurve2,
                                    meshobject_or_utbm, alongwire, alongwireI, dsangle, Maxsideslipturningfactor,
                                    mandrelradius, sideslipturningfactorZ, maxlength, outputfilament):
    utbm = (meshobject_or_utbm if isinstance(meshobject_or_utbm, UsefulBoxedTriangleMesh)
            else UsefulBoxedTriangleMesh(meshobject_or_utbm.Mesh))

    drivecurve1 = (sketchplane_or_drivecurve1 if isinstance(sketchplane_or_drivecurve1, DriveCurve)
                       else makedrivecurve(sketchplane_or_drivecurve1, utbm, mandrelradius))
    drivecurve2 = (sketchplane_or_drivecurve2 if isinstance(sketchplane_or_drivecurve2, DriveCurve)
                     else makedrivecurve(sketchplane_or_drivecurve2, utbm, mandrelradius))

    #print(f'From directedgeodesic util, drivecurvestart is: {drivecurvestart}')
    #print(f'From directedgeodesic util, drivecurveend is: {drivecurveend}')

    gbStart = drivecurve1.startalongangle(alongwire, dsangle)
    fLRdirection = 1 if ((dsangle + 360)%360) < 180.0 else -1
    if combofoldbackmode != 0:
        fLRdirection = -fLRdirection
        
    #print(f'drivecurve.drivebars is: \n{drivecurvestart.drivebars} \ndrivecurve.tridrivebarsmap is: \n{drivecurvestart.tridrivebarsmap}')
    #print("doing alongwire %.2f foldback=%d  alongwireI %.2f" % (alongwire, fLRdirection, alongwireI or 0))
    gbs = drivegeodesicRI_multidrivecurve(gbStart, drivecurve1.drivebars, drivecurve1.tridrivebarsmap,
                                          drivecurve2.drivebars, drivecurve2.tridrivebarsmap, LRdirection=fLRdirection,
                                          sideslipturningfactor=sideslipturningfactorZ, maxlength=maxlength)
    if gbs[-1] == None:
        return gbs, 0, -1, None, None
        
    alongwirelanded = drivecurve2.endalongposition(gbs[-1])
    alongwirelanded, angcross = drivecurve2.endalongpositionA(gbs[-1])
    ...
    gbStartI = drivecurve1.startalongangle(alongwireI, dsangleI)
    gbsI = drivegeodesicRI(gbStartI, drivebarsB, tridrivebarsmapB, sideslipturningfactor=sideslipturningfactor,
    ...
    for j in range(2):
        ...
        gbsIN = drivegeodesicRI(gbStartI, drivebarsB, tridrivebarsmapB, sideslipturningfactor=sideslipturningfactor,
                                LRdirection=LRdirectionI, MAX_SEGMENTS=len(gbs))
        if gbsIN[-1] != None:
            gbsI = gbsIN
...
def directedgeodesic(combofoldbackmode, sketchplane_or_drivecurve, meshobject_or_utbm, 
    ...
    drivecurve = (sketchplane_or_drivecurve if isinstance(sketchplane_or_drivecurve, DriveCurve)
                  else makedrivecurve(sketchplane_or_drivecurve, utbm, mandrelradius))
    print(f'From directedgeodesic util, drivecurve is: {drivecurve}')
    gbStart = drivecurve.startalongangle(alongwire, dsangle)
    fLRdirection = 1 if ((dsangle + 360)%360) < 180.0 else -1
    if combofoldbackmode != 0:
        fLRdirection = -fLRdirection
        
    print(f'drivecurve.drivebars is: \n{drivecurve.drivebars} \ndrivecurve.tridrivebarsmap is: \n{drivecurve.tridrivebarsmap}')
    #print("doing alongwire %.2f foldback=%d  alongwireI %.2f" % (alongwire, fLRdirection, alongwireI or 0))
    gbs = drivegeodesicRI(gbStart, drivecurve.drivebars, drivecurve.tridrivebarsmap, LRdirection=fLRdirection,
                          sideslipturningfactor=sideslipturningfactorZ, maxlength=maxlength)
    if gbs[-1] == None:
        if outputfilament:
    ...
    gbarT1.gbBackbarC, gbarT1.gbForebarC = gbarT1.gbForebarC, gbarT1.gbBackbarC
    gbsjoined = gbs[:dseg+1] + [ GBarC(gb.bar, gb.lam, not gb.bGoRight)  for gb in gbsI[-2:0:-1] ] + [ gbarT1 ]
    #makebicolouredwire(gbsjoined, outputfilament, colfront=(1.0,0.0,0.0) if fLRdirection == -1 else (0.0,0.0,1.0), 
