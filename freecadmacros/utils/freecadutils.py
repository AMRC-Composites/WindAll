...
from barmesh.basicgeo import Along
...
def getlabelofselectedlam():
    for s in sel:
        if hasattr(s, "Windall"):
            return s.Label
    return ""
...
def showdrivebarscurve(drivebars, lab):
    ...
    epts = [Along(lam, bar.nodeback.p, bar.nodefore.p)
...
def showdrivebarsmesh(drivebars, lab):
    ...
    tbarfacets = [facetbetweenbars(drivebars[i][0], drivebars[i+1][0])
                  for i in range(len(drivebars)-1)]
